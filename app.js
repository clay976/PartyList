...
var FUCKTHISSHIT = require ('./track.json');
var host;
...
console.log (FUCKTHISSHIT.tracks.items[0].id);
        ...
        request.get(options, function(error, response, body) {
          ...
          query.search (host, {'host':host}, function(found){ 
            if (found != null){
              ...
              update.updater (host, found, updateInfo, function(error){
...
app.get('/refresh_token', function refreshToken (req, res) {
  ...
  var refresh_token;
  query.search (host,doc, function(docum){
  ...
  request.post(authOptions, function(error, response, body) {
    if (!error && response.statusCode === 200) {
      ...
      update.updater (host, documUpdate, updateInfo, function(error){
...
app.post('/createPlaylist', function(req, res){
  ...
  validateToken.checkToken (host, function(tokenValid, docFound){
  var options = {
...
app.post('/message', function(req, res){
  ...
  request.get(options, function(error, response, body) {
    ...
    //playlist.addTack(user.id, trackID, playlistID);
  });   
});
